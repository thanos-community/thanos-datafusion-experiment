...
The end-to-end suites exercise the reader through its public APIs with isolated
temporary fixture directories, caches, gRPC listeners, and metrics listeners. Run both suites
...
`cargo test --test e2e` reports named Arrow Flight SQL cases, including ordering, label/time
filters, downsampled rows, special floating-point values, and SQL errors. The Go StoreAPI suite
runs with `-v`, so each Info, Series, Label API, and unsupported-request case is shown
individually. On a Rust e2e failure, the fixture directory and reader stderr log are retained and
